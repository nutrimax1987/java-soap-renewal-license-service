
package com.amnetgov.jaxws;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlRootElement;
import javax.xml.bind.annotation.XmlType;

/**
 * This class was generated by Apache CXF 3.0.5
 * Tue Apr 12 18:21:06 IDT 2016
 * Generated source version: 3.0.5
 */

@XmlRootElement(name = "SolicitarIdentificadorSeguimiento", namespace = "http://amnetgov.com/")
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "SolicitarIdentificadorSeguimiento", namespace = "http://amnetgov.com/", propOrder = {"UsuarioServicio", "ContrasenaServicio", "NumeroProceso", "RolEspecialista", "Especialista", "Inicio"})

public class SolicitarIdentificadorSeguimiento {

    @XmlElement(name = "UsuarioServicio", namespace = "http://amnetgov.com/")
    private java.lang.String UsuarioServicio;
    @XmlElement(name = "ContrasenaServicio", namespace = "http://amnetgov.com/")
    private java.lang.String ContrasenaServicio;
    @XmlElement(name = "NumeroProceso", namespace = "http://amnetgov.com/")
    private long NumeroProceso;
    @XmlElement(name = "RolEspecialista", namespace = "http://amnetgov.com/")
    private java.lang.String RolEspecialista;
    @XmlElement(name = "Especialista", namespace = "http://amnetgov.com/")
    private java.lang.String Especialista;
    @XmlElement(name = "Inicio", namespace = "http://amnetgov.com/")
    private boolean Inicio;

    public java.lang.String getUsuarioServicio() {
        return this.UsuarioServicio;
    }

    public void setUsuarioServicio(java.lang.String newUsuarioServicio)  {
        this.UsuarioServicio = newUsuarioServicio;
    }

    public java.lang.String getContrasenaServicio() {
        return this.ContrasenaServicio;
    }

    public void setContrasenaServicio(java.lang.String newContrasenaServicio)  {
        this.ContrasenaServicio = newContrasenaServicio;
    }

    public long getNumeroProceso() {
        return this.NumeroProceso;
    }

    public void setNumeroProceso(long newNumeroProceso)  {
        this.NumeroProceso = newNumeroProceso;
    }

    public java.lang.String getRolEspecialista() {
        return this.RolEspecialista;
    }

    public void setRolEspecialista(java.lang.String newRolEspecialista)  {
        this.RolEspecialista = newRolEspecialista;
    }

    public java.lang.String getEspecialista() {
        return this.Especialista;
    }

    public void setEspecialista(java.lang.String newEspecialista)  {
        this.Especialista = newEspecialista;
    }

    public boolean getInicio() {
        return this.Inicio;
    }

    public void setInicio(boolean newInicio)  {
        this.Inicio = newInicio;
    }

}

